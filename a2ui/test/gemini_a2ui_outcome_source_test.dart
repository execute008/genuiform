// Tests for GeminiA2uiOutcomeSource.
//
...
///
/// Matches the schema produced by [a2uiOutcomeJsonSchema].
  ...
  group('GeminiA2uiOutcomeSource', () {
    ...
    group('forwards prompt and schema to LlmClient.generate', () {
      test('systemPrompt contains the outcome id', () async {
        ...
        final source = GeminiA2uiOutcomeSource(client: fake);
      ...
      test('uses responseJsonSchema (not legacy responseSchema)', () async {
        ...
        final inv = fake.invocations.first;
        // The source must use the newer `responseJsonSchema` field, which
        ...
        expect(
          ...
          equals(jsonEncode(a2uiOutcomeJsonSchema())),
        );
      });
      ...
      test('custom model is forwarded', () async {
        ...
        final source = GeminiA2uiOutcomeSource(
          client: fake,
      ...
      test('rejects -latest model aliases that do not support responseJsonSchema',
        ...
        expect(
          () => GeminiA2uiOutcomeSource(client: fake, model: 'gemini-flash-latest'),
          throwsA(
        ...
        expect(
          () => GeminiA2uiOutcomeSource(client: fake, model: 'gemini-pro-latest'),
          throwsA(isA<ArgumentError>()),
        );
      });
    });
    ...
    group('propagates LlmClientError subtypes without swallowing', () {
      test('AuthError propagates', () async {
        ...
        final source = GeminiA2uiOutcomeSource(client: client);

        await expectLater(
          source.emit(outcomeId: 'x', handoff: null, summary: ''),
      ...
      test('preserved error is identical instance (not wrapped)', () async {
        ...
        Object? caught;
        try {
          await source.emit(outcomeId: 'x', handoff: null, summary: '').toList();
        } catch (e) {
          caught = e;
        }
    ...
    group('accumulates deltas from a streaming LlmClient', () {
      test('yields exactly two envelopes when the response arrives as N deltas',
        ...
        // SSE deltas arriving over time. The source must accumulate them
        // before parsing.
      ...
      test('does not throw on partial-JSON intermediate deltas', () async {
        ...
        // concatenation parses. The current per-chunk-jsonDecode source
    ...
    // `"weight":1.01121111…`). The buffer-then-parse source has no notion of
    ...
    // SchemaError originating at the source, and bounds memory.
